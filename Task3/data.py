...
def harmonic_data(E_infty, E, eta, n, omega, A):
    ...
    for i in range(len(t)-1):
        ...
        gamma[i+1] = gamma[i] + dt * E / eta * ( eps[i] - gamma[i] )  # equation 15     
        sig[i+1] = E_infty * eps[i+1] + E * ( eps[i+1] - gamma[i+1] ) # equation 9 
        
    # Für RNNs in Keras müssen die Eingabedaten 3-dimensional sein mit der Form: (batch_size, timesteps, features)
       
    eps = tf.constant(eps, dtype=tf_dt)
    eps = tf.expand_dims(eps, axis = 0)
    eps = tf.expand_dims(eps, axis = 2)  # shape (1,100,1)  
